<template>
  <div>
    <div>
      ...
      <el-row :gutter="10" class="mb8">
        <div v-if="buttonAuthority" style="display: inline-block">
          ...
          <el-button size="small" @click="handleAttention" type="primary">重点关注</el-button>
          <el-button size="small" @click="handleRandomy" type="primary">纳入抽查</el-button>
        </div>
      ...
      >
        ...
        <el-table-column label="查重" width="80px">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" @click="handleRechecking(scope.row)">查重</el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="80" fixed="right">
          <template slot-scope="scope" v-if="buttonAuthority">
            <el-button size="mini" type="primary" @click="handleCheck(scope.row)">审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
    ...
    >
      ...
      <div class="manage-info">
        <div class="info-item">
          ...
          <div class="info-item-quare">
            ...
            <span class="info-item-content">{{ manageInfo.inspectAreaName }}</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item-quare">
        ...
        <div class="info-item">
          <div class="info-item-quare">
            ...
            <span class="info-item-content">{{ manageInfo.tipOffContent }}</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item-quare">
        ...
        <div class="info-item">
          <div class="info-item-quare">
            ...
            <span class="info-item-content">{{ manageInfo.pollutionTypeMain }}</span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item-quare">
            <span class="info-item-title">次要污染类型：</span>
            <span class="info-item-content">{{ manageInfo.pollutionTypeSub }}</span>
          </div>
        </div>
      ...
      >
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="是否修改内容">
              <el-checkbox-group v-model="reviewContent" @change="handleReviewChange">
        ...
        <el-row :gutter="24" v-if="industryTypeBoolean == true">
          <el-col :span="12">
            <el-form-item label="行业类型修正：" prop="industryTypeReview">
              <el-select v-model="dialogParams.industryTypeReview" placeholder="请选择行业类型">
                <el-option
        ...
        <el-row :gutter="24" v-if="addressBoolean == true">
          <el-col :span="24">
            <el-form-item label="举报地址修正：" class="meet-address" prop="addressReview">
              <el-select
              ...
              <el-select v-model="dialogParams.countyRevire" filterable placeholder="请选择区县">
                <el-option
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="主要污染类型：" v-if="pollutionLevelBoolean == true">
              <div class="main-task-box">
                ...
                <el-input
                  ...
                  v-model="dialogParams.mainPollutionOtherReview"
                  placeholder
                ></el-input>
              </div>
            </el-form-item>
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="次要污染类型：" v-if="pollutionLevelBoolean == true">
              ...
              <div class="pollute-box">
                <div class="pollute-item" v-for="(item, index) in pollutantArryTwo" :key="index">
                  ...
                  <el-checkbox-group
                    v-model="dialogParams.subPollutionLevelTwoReview"
                    v-for="(itemb, indexb) in item.children"
                    @change="handlePollutionChange"
                    :key="indexb"
                    size="small"
                  >
                    <el-checkbox-button :label="itemb.value">{{ itemb.label }}</el-checkbox-button>
                  </el-checkbox-group>
                  <el-input
        ...
        <el-row :gutter="24" v-if="longitudeBoolean == true">
          <el-col :span="12">
            <el-form-item label="经度修正：" prop="longitude">
              <el-input placeholder="请输入经度" v-model="dialogParams.longitudeReview"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="纬度修正：" prop="latitude">
              <el-input placeholder="请输入纬度" v-model="dialogParams.latitudeReview"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="24">
          ...
          <el-col :span="12" v-if="dialogParams.weatherAccept === '0'">
            <el-form-item label="不受理原因：" prop="handIdea">
              <el-input v-model="dialogParams.handIdea" placeholder="请输入不受理原因"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <!-- <el-row :gutter="24">
        ...
        <el-row :gutter="24">
          <el-col :span="24" v-if="dialogParams.repeatId">
            <el-form-item label="重复举报信息：" prop="repeatNums">
              <el-button
        ...
        <el-row :gutter="24">
          <el-col :span="12" v-if="dialogParams.weatherAccept === '1'">
          ...
          <el-col
            :span="12"
            v-if="dialogParams.weatherTransfer === '1'&&dialogParams.weatherAccept === '1'"
          >
    ...
    <el-dialog
      ...
      width="80%"
      :before-close="handleDuplicateClose"
      append-to-body
      :close-on-click-modal="false"
    ...
    >
        ...
        <el-form :model="queryLogParams" label-width="100px">
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="举报时间：">
                <el-date-picker
                  v-model="logIssueDate"
                  type="daterange"
                  range-separator="至"
                  start-placeholder="开始日期"
                  end-placeholder="结束日期"
                  value-format="yyyy-MM-dd"
                  @change="issueLogDateChange"
                ></el-date-picker>
              </el-form-item>
            </el-col>
      ...
      <div class="dialogFormFooter">
        ...
        <el-button type="primary" @click="handleDuplicateClose">关闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      dialogParams: {
        mainTaskArray: [],
        mainPollutionOtherReview: "",
        pollutionLevelOneReview: "",
        ...
        subPollutionLevelOneReivew: [],
        subPollutionLevelTwoReview: [],
      ...
      dialogRules: {
        ...
        mainTaskArray: [
          { required: true, message: "请选择主要污染类型", trigger: "blur" }
        ]
      },
      countyArry: [],
      multipleSelection: [],
  ...
  computed: {
    ...
    handleCheckShow() {
      return function(value) {
        return this.dialogParams.subPollutionLevelTwoReview.indexOf(value) > -1
          ? true
          : false;
      };
    }
  ...
  methods: {
    ...
    handlePollutionChange() {
      ...
      for (const item in this.pollutantArryTwo) {
        ...
        for (const itemb in subject.children) {
          ...
          if (
            this.dialogParams.subPollutionLevelTwoReview.indexOf(
              subjectb.value
            ) > -1
          ) {
      ...
      this.dialogParams.subPollutionLevelOneReview = [
        ...new Set(itemIndexArry)
      ];
    },
      ...
      this.$refs.dialogForm.validate(async valid => {
        if (valid) {
          ...
          if (
            ...
            this.dialogParams.subPollutionLevelTwoReview.length == 0
          ) {
          ...
          this.dialogParams.subPollutionLevelOneReview = this.dialogParams.subPollutionLevelOneReview.join(
            ","
          );
          this.dialogParams.subPollutionLevelTwoReview = this.dialogParams.subPollutionLevelTwoReview.join(
            ","
          );
    ...
    getAmendDecide() {
      ...
      if (len == 0) {
        ...
        const levelString = this.dialogParams.subPollutionLevelTwoReview.join(
          ","
        );
        const amendString = this.amend.subPollutionLevelTwoReview.join(",");
        if (
          this.reviewContent.indexOf("3") > -1 &&
    ...
    getCheckCommonContent(data) {
      ...
      this.dialogParams.industryTypeReview = data.industryTypeReview
        ? data.industryTypeReview
        : data.industryType;
      this.dialogParams.tipOffContentReivew = data.tipOffContentReivew
        ? data.tipOffContentReivew
        : data.tipOffContent;
      this.dialogParams.cityReview = data.cityReview
        ? data.cityReview
        : data.city;
      this.dialogParams.countyRevire = data.countyRevire
        ? data.countyRevire
        : data.county;
      this.dialogParams.addressReview = data.addressReview
        ? data.addressReview
        : data.address;
      if (data.subPollutionLevelOneReview) {
        this.dialogParams.subPollutionLevelOneReview = data.subPollutionLevelOneReview.split(
          ","
        );
      } else if (data.subPollutionLevelOne) {
        this.dialogParams.subPollutionLevelOneReview = data.subPollutionLevelOne.split(
          ","
        );
      } else {
        this.dialogParams.subPollutionLevelOneReview = [];
      }
      if (data.subPollutionLevelTwoReview) {
        this.dialogParams.subPollutionLevelTwoReview = data.subPollutionLevelTwoReview.split(
          ","
        );
      } else if (data.subPollutionLevelTwo) {
        this.dialogParams.subPollutionLevelTwoReview = data.subPollutionLevelTwo.split(
          ","
        );
      } else {
        this.dialogParams.subPollutionLevelTwoReview = [];
      }
      this.dialogParams.mainTaskArray = data.pollutionLevelOneReview
        ? [data.pollutionLevelOneReview, data.pollutionLevelTwoReview]
        : [data.pollutionLevelOne, data.pollutionLevelTwo];
      if (data.mainPollutionOtherReview) {
        this.showMainTaskOtherInput = true;
        this.dialogParams.mainPollutionOtherReview =
          data.mainPollutionOtherReview;
      } else if (data.mainPollutionOther) {
        this.showMainTaskOtherInput = true;
        this.dialogParams.mainPollutionOtherReview = data.mainPollutionOther;
      }
      this.dialogParams.longitudeReview = data.longitudeReview
        ? data.longitudeReview
        : data.longitude;
      this.dialogParams.latitudeReview = data.latitudeReview
        ? data.latitudeReview
        : data.latitude;

      this.amend.industryTypeReview = data.industryTypeReview
        ? undefined
        : data.industryType;
      this.amend.tipOffContentReivew = data.tipOffContentReivew
        ? undefined
        : data.tipOffContent;
      this.amend.cityReview = data.cityReview ? undefined : data.city;
      this.amend.countyRevire = data.countyRevire ? undefined : data.county;
      this.amend.addressReview = data.addressReview ? undefined : data.address;
      this.amend.subPollutionLevelOneReview = data.subPollutionLevelOneReview
        ? data.pollutionLevelOne.split(",")
        : [];
      this.amend.subPollutionLevelTwoReview = data.subPollutionLevelTwoReview
        ? data.pollutionLevelTwo.split(",")
        : [];
      this.amend.longitudeReview = data.longitudeReview
        ? undefined
        : data.longitude;
      this.amend.latitudeReview = data.latitudeReview
        ? undefined
        : data.latitude;
      ...
      if (data.repeatId) {
        this.getRepeatNumList(data.repeatId);
      }
      ...
      this.dialogParams.pollutionWaterAddReview = data.pollutionWaterAddReview
        ? data.pollutionWaterAddReview
        : data.pollutionWaterAdd;
      this.dialogParams.pollutionAirAddReview = data.pollutionAirAddReview
        ? data.pollutionAirAddReview
        : data.pollutionAirAdd;
      this.dialogParams.pollutionSolidAddReivew = data.pollutionSolidAddReivew
        ? data.pollutionSolidAddReivew
        : data.pollutionSolidAdd;
      this.dialogParams.pollutionOceanAddReview = data.pollutionOceanAddReview
        ? data.pollutionOceanAddReview
        : data.pollutionOceanAdd;
      this.dialogParams.pollutionEnvironmentAddReview = data.pollutionEnvironmentAddReview
        ? data.pollutionEnvironmentAddReview
        : data.pollutionEnvironmentAdd;
      this.dialogParams.pollutionRadiationAddReview = data.pollutionRadiationAddReview
        ? data.pollutionRadiationAddReview
        : data.pollutionRadiationAdd;
      this.dialogParams.pollutionNoiseAddReview = data.pollutionNoiseAddReview
        ? data.pollutionNoiseAddReview
        : data.pollutionNoiseAdd;
      this.dialogParams.pollutionOtherAddReview = data.pollutionOtherAddReview
        ? data.pollutionOtherAddReview
        : data.pollutionOtherAdd;
    },
    ...
    handleMainTaskChange: function(value) {
      ...
      this.dialogParams.pollutionLevelOneReview = value[0];
      this.dialogParams.pollutionLevelTwoReview = value[1];
    },
    async getCheckContent(id) {
      const reponse = await acceptCheckInfo({ id, lockFlag: true });
      this.initWebSocket(id, this.userInfo.name);
      // const reponse = await acceptCheckInfo({ id });
      if (reponse.code == 500) {
        this.open = false;
        return;
      }
      // sessionStorage.setItem("lockFlagId", id);
      this.open = true;
      const subject = reponse.data;
    ...
    async handleDialogClose() {
      // const reponse = await acceptDeleteLock({ id: this.dialogParams.id });
      // sessionStorage.removeItem("lockFlagId");
    ...
    issueLogDateChange(picker) {
      if (picker == null) {
        this.queryLogParams.beginTipTime = undefined;
        this.queryLogParams.endTipTime = undefined;
      } else {
        this.queryLogParams.beginTipTime = picker[0];
        this.queryLogParams.endTipTime = picker[1];
      }
    },
    async getRepeatList() {
      this.logLoading = true;
      const reponse = await intelRecheckingList(this.queryLogParams);
      reponse.data &&
    ...
    handleRechecking(data) {
      const { id } = data;
      this.duplicateOpen = true;
      this.queryLogParams.id = id;
      this.getRepeatList();
    },
    onDialogQuery() {
      this.getRepeatList();
    ...
    onDialogClear() {
      ...
      this.queryLogParams.beginTipTime = undefined;
      this.queryLogParams.endTipTime = undefined;
      this.logIssueDate = undefined;
    },
    async handleAddRepeatClick() {
      const data = this.multipleLogSelection;
      if (data.length) {
        const cIdsArry = [];
        const flagArry = [];
        for (const item in data) {
          const subject = data[item];
          cIdsArry.push(subject.id);
          flagArry.push(subject.handleState);
        }
        const cIds = cIdsArry.join(",");
        const flagStr = flagArry.join(",");
        let flag;
        let str;
        if (
          flagStr.indexOf("5") > -1 ||
          ...
          flag = 3;
          str =
            "当前选中数据存在已转办或以上状态数据，确定将其他数据一并转办或办理吗？";
        } else if (flagStr.indexOf("3") > -1 || flagStr.indexOf("4") > -1) {
          flag = 2;
          str =
            "当前选中数据存在受理数据，确定将所选其他数据一并受理吗？（未审核数据会自动生成受理编号并跟随自动受理）";
        } else if (flagStr.indexOf("2") > -1) {
          flag = 1;
          str =
            "当前选中数据存在不受理数据，确定将其他数据和查重数据跟随不受理吗？";
        } else {
          flag = 0;
          str =
            "当前选中数据均未审核，确定将所选数据合并到进行查重的信访件一并处理吗？";
        }
        this.$confirm(`${str}`, "确认信息", {
          ...
          this.$confirm("请确认操作", "确认信息", {
            distinguishCancelAndClose: true,
            confirmButtonText: "确认",
            cancelButtonText: "取消"
          }).then(async () => {
            this.handleAddRepeatMethod(cIds, flag);
          });
        });
        //请确认操作
      } else {
        this.$message.error(`请选择需要添加重复的数据`);
        return;
      }
    },
    async handleAddRepeatMethod(cIds, flag) {
      const reponse = await manuallyMerge({
        mainId: this.queryLogParams.id,
        cIds,
        flag
      });
      if (reponse.code == 200) {
    ...
    handleDuplicateClose() {
      ...
      this.queryLogParams.id = undefined;
    },
    checkSelectable(row) {
      if (row.checked == true) {
        return true;
    ...
    handleLogSelectionChange(data) {
      this.multipleLogSelection = data;
      const subject = data[0];
      if (data.length) {
        if (subject.handleState == 2) {
          ...
          subject.handleState == 3 ||
          subject.handleState == 4 ||
          subject.handleState == 5 ||
          subject.handleState == 6 ||
          subject.handleState == 7 ||
          subject.handleState == 8 ||
          subject.handleState == 9
        ) {
          this.logList.map(item => {
            if (item.handleState == 2) {
              item.checked = false;
            }
            return item;
          });
