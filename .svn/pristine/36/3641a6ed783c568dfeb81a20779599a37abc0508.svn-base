<template>
  <div>
    ...
    <el-table v-loading="loading" :data="List" border :stripe="true">
      ...
      <el-table-column label="书记表态" width="200px">
        <template slot-scope="scope">
          <span
            class="word-icon"
            @click="handleFileDialog(scope.row.id, 'secretaryStateFileIds')"
          ></span>
        </template>
      </el-table-column>
      <el-table-column label="回应材料" width="200px">
        <template slot-scope="scope">
          <span
            class="word-icon"
            @click="handleFileDialog(scope.row.id, 'responseMaterialFileIds')"
          ></span>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item label="书记表态：" prop="secretaryStateFileIds">
              <upload
                ref="uploadFile1"
                accept=".doc, .docx, .pdf"
                :limit="5"
                :fileId="dialogParams.secretaryStateFileIds"
                fromKey="secretaryStateFileIds"
          ...
          <el-col :span="12">
            <el-form-item label="回应材料：" prop="responseMaterialFileIds">
              <upload
                ref="uploadFile2"
                accept=".doc, .docx, .pdf"
                :limit="5"
                :fileId="dialogParams.responseMaterialFileIds"
                fromKey="responseMaterialFileIds"
                @setUrlPath="setUrlPath"
                @deleteServeFile="deleteServeFile"
              ></upload>
            </el-form-item>
    ...
    >
      ...
      <div class="manage-info">
        ...
        <div class="info-item">
          <div class="info-item-quare">
            <span class="info-item-title">书记表态：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.secretaryStateFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
          <div class="info-item-quare">
            <span class="info-item-title">回应材料：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.responseMaterialFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
  ...
  data() {
    return {
      ...
      dialogParams: {
        ...
        attendees: undefined,
        secretaryStateFileIds: [],
        responseMaterialFileIds: [],
        photoFileIds: [],
        otherFiles: []
      },
      dialogRules: {
        ...
        secretaryStateFileIds: [{ required: true, message: "请上传书记表态" }],
        responseMaterialFileIds: [
          { required: true, message: "请上传回应材料" }
        ],
      ...
      manageInfo: {
        ...
        attendees: undefined,
        address: undefined,
        secretaryStateFileArray: [],
        responseMaterialFileArray: [],
        photoFileArray: [],
        otherFileArray: []
      }
    };
  ...
  methods: {
    ...
    dailogSubmit() {
      this.$refs.dialogForm.validate(async valid => {
        if (valid) {
          this.dialogParams.secretaryStateFileIds = this.dialogParams.secretaryStateFileIds.join(
            ","
          );
          this.dialogParams.responseMaterialFileIds = this.dialogParams.responseMaterialFileIds.join(
            ","
          );
    ...
    async getDialogInfo(id) {
      ...
      this.dialogParams.secretaryStateFileIds = subject.secretaryStateFileIds.split(
        ","
      );
      this.dialogParams.responseMaterialFileIds = subject.responseMaterialFileIds.split(
        ","
      );
    ...
    async handleSee({ id }) {
      ...
      subject.secretaryStateFileIds &&
        this.getMetSingleMessagInfo(
          subject.secretaryStateFileIds,
          "secretaryStateFileArray"
        );
      subject.responseMaterialFileIds &&
        this.getMetSingleMessagInfo(
          subject.responseMaterialFileIds,
          "responseMaterialFileArray"
        );
    ...
    handleDialogLookClose() {
      this.lookOpen = false;
      this.manageInfo.secretaryStateFileArray = undefined;
      this.manageInfo.responseMaterialFileArray = undefined;
