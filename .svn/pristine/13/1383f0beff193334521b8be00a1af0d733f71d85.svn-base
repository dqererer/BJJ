    ...
    <div class="clue-info">
      ...
      <div class="clue-item clue-item-paper">
        <div class="clue-item-quare clue-item-inspect">
          ...
          <div class="clue-item-file">
            <div class="clue-item-file-group" v-for="(item,index) in manageInfo.audioArry" :key="index">
              <span class="accounChunk">{{item.file_name}}</span>
              <el-button size="mini" @click="handleKnowPreviewClick(item.id,item.save_path,item.file_name,item.object_id)">预览</el-button>
              <el-button
                size="mini"
...
export default {
  ...
  methods: {
    ...
    async getInfo() {
      ...
      subject.audioId && this.getSingleMessagInfo(subject.audioId);
