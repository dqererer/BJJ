<template>
  <div>
    ...
    <el-table v-loading="loading" :data="List" border :stripe="true">
      ...
      <el-table-column label="组长讲话稿" width="200px">
        <template v-slot="scope">
          <span
            class="word-icon"
      ...
      <el-table-column label="副组长讲话稿" width="200px">
        <template v-slot="scope">
          <span
            class="word-icon"
      ...
      <el-table-column label="省领导讲话稿" width="200px">
        <template v-slot="scope">
          <span
            class="word-icon"
      ...
      <el-table-column label="新闻通稿" width="200px">
        <template v-slot="scope">
          <span
            class="word-icon"
    ...
    >
      ...
      >
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12">
            <el-form-item label="副组长讲话稿：" prop="deputyLeaderSpeechFileIds">
              <upload
                ref="uploadFile2"
                accept=".doc, .docx, .pdf"
                :limit="5"
    ...
    >
      ...
      <div class="manage-info">
        ...
        <div class="info-item">
          <div class="info-item-quare">
            <span class="info-item-title">组长讲话稿：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.leaderSpeechFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
          <div class="info-item-quare">
            <span class="info-item-title">副组长讲话稿：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.deputyLeaderSpeechFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
        ...
        <div class="info-item">
          <div class="info-item-quare">
            <span class="info-item-title">省领导讲话稿：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.provincialLeadersFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
          <div class="info-item-quare">
            <span class="info-item-title">新闻通稿：</span>
            <div class="info-item-content">
              <span
                class="accounChunk"
                @click="handleKnowDownloadClick(item.id,item.save_path,item.file_name,item.object_id)"
                v-for="(item,index) in manageInfo.pressReleaseFileArray"
                :key="index"
              >{{item.file_name}}</span>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      manageInfo: {
        ...
        attendees: undefined,
        address: undefined,
        leaderSpeechFileArray: [],
        deputyLeaderSpeechFileArray: [],
        provincialLeadersFileArray: [],
        pressReleaseFileArray: [],
        vedioFileArray:[],
        photoFileArray:[],
        otherFileArray:[]
      }
  ...
  methods: {
    ...
    async handleSee({ id }) {
      ...
      subject.leaderSpeechFileIds &&
        this.getMetSingleMessagInfo(
          subject.leaderSpeechFileIds,
          "leaderSpeechFileArray"
        );
      subject.deputyLeaderSpeechFileIds &&
        this.getMetSingleMessagInfo(
          subject.deputyLeaderSpeechFileIds,
          "deputyLeaderSpeechFileArray"
        );
      subject.provincialLeadersFileIds &&
        this.getMetSingleMessagInfo(
          subject.provincialLeadersFileIds,
          "provincialLeadersFileArray"
        );
      subject.pressReleaseFileIds &&
        this.getMetSingleMessagInfo(
          subject.pressReleaseFileIds,
          "pressReleaseFileArray"
        );
    ...
    handleDialogLookClose() {
      this.lookOpen = false;
      this.manageInfo.leaderSpeechFileArray = undefined;
      this.manageInfo.deputyLeaderSpeechFileArray = undefined;
      this.manageInfo.provincialLeadersFileArray = undefined;
      this.manageInfo.pressReleaseFileArray = undefined;
