<template>
  <div class="information-inner information-change">
    ...
    <div class="tab-list-content">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="督察批次：">
                <el-select v-model="queryParams.batchId" placeholder="请选择督察批次">
                  <el-option
            ...
            <el-col :span="8">
              <el-form-item label="督察地区：">
                <el-select v-model="queryParams.city" filterable placeholder="请选择督察地区">
                  <el-option
      ...
      <div class="list-box">
        <el-button type="primary" size="mini" v-if="buttonShow" @click="handleOverseersClick">展开边督边改</el-button>
        <el-table v-loading="loading" :data="List" border :stripe="true" v-else>
          ...
          ></el-table-column>
          <el-table-column label="收到举报数量(件)">
            <el-table-column label="来电" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipCall, {
                      state: 1,
                      tipType: 0,
                    })
                  "
                >{{ scope.row.tipCall }}</span>
              </template>
            </el-table-column>
            <el-table-column label="来信" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipLetter, {
                      state: 1,
                      tipType: 1,
                    })
                  "
                >{{ scope.row.tipLetter }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合计" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipAll, { state: 1 })
                  "
                >{{ scope.row.tipAll }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="举报包含后续(件)">
            <el-table-column label="来电" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipCallWithLater, {
                      state: 1,
                      tipType: 0,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.tipCallWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="来信" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipLetterWithLater, {
                      state: 1,
                      tipType: 1,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.tipLetterWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合计" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.tipAllWithLater, {
                      state: 1,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.tipAllWithLater }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="不受理(件)" width="100">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleSee(scope.row.city, scope.row.unAccept, {
                    state: 2,
                    followFlag: 0,
                  })
                "
              >{{ scope.row.unAccept }}</span>
            </template>
          </el-table-column>
          <el-table-column label="跟随不受理(件)" width="120">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleSee(scope.row.city, scope.row.unAcceptFollow, {
                    state: 2,
                    followFlag: 1,
                  })
                "
              >{{ scope.row.unAcceptFollow }}</span>
            </template>
          </el-table-column>
          <el-table-column label="受理且暂不转办(件)">
            <el-table-column label="来电" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptNCall, {
                      state: 3,
                      tipType: 0,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.acceptNCall }}</span>
              </template>
            </el-table-column>
            <el-table-column label="来信" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptNLetter, {
                      state: 3,
                      tipType: 1,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.acceptNLetter }}</span>
              </template>
            </el-table-column>
            <el-table-column label="跟随" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptNFollow, {
                      state: 3,
                      followFlag: 1,
                    })
                  "
                >{{ scope.row.acceptNFollow }}</span>
              </template>
            </el-table-column>
            <!-- <el-table-column prop="acceptAll" label="合计" width="80"></el-table-column> -->
          </el-table-column>
          <el-table-column label="受理且转办(件)">
            <el-table-column label="来电" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptCall, {
                      state: 4,
                      tipType: 0,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.acceptCall }}</span>
              </template>
            </el-table-column>
            <el-table-column label="来信" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptLetter, {
                      state: 4,
                      tipType: 1,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.acceptLetter }}</span>
              </template>
            </el-table-column>
            <el-table-column label="跟随" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptFollow, {
                      state: 4,
                      followFlag: 1,
                    })
                  "
                >{{ scope.row.acceptFollow }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合计" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptAll, { state: 4 })
                  "
                >{{ scope.row.acceptAll }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="受理包含后续(件)">
            <el-table-column label="来电" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptCallWithLater, {
                      state: 3,
                      tipType: 0,
                      followFlag: 0,
                      symbol: 1,
                  ...
                  "
                >{{ scope.row.acceptCallWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="来信" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptLetterWithLater, {
                      state: 3,
                      tipType: 1,
                      followFlag: 0,
                      symbol: 1,
                  ...
                  "
                >{{ scope.row.acceptLetterWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="跟随" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptFollowWithLater, {
                      state: 3,
                      followFlag: 1,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.acceptFollowWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合计" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.acceptAllWithLater, {
                      state: 3,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.acceptAllWithLater }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="交办数量(件)" width="110px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleSee(scope.row.city, scope.row.transferTow, {
                    state: 6,
                    followFlag: 0,
                  })
                "
              >{{ scope.row.transferTow }}</span>
            </template>
          </el-table-column>
          <el-table-column label="转办（包含后续）">
            <el-table-column label="转办" width="110px">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.transferWithLater, {
                      state: 5,
                      followFlag: 0,
                      symbol: 1,
                    })
                  "
                >{{ scope.row.transferWithLater }}</span>
              </template>
            </el-table-column>
            <el-table-column label="跟随" width="120px">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(
                      scope.row.city,
                      scope.row.transferFollowWithLater,
                      { state: 5, followFlag: 1, symbol: 1 }
                    )
                  "
                >{{ scope.row.transferFollowWithLater }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="未办结(件)" width="90px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleSee(scope.row.city, scope.row.unDone, {
                    state: 7,
                    followFlag: 0,
                  })
                "
              >{{ scope.row.unDone }}</span>
            </template>
          </el-table-column>
          <el-table-column label="阶段性办结(件)" width="120px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleSee(scope.row.city, scope.row.partDone, {
                    state: 8,
                    followFlag: 0,
                  })
                "
              >{{ scope.row.partDone }}</span>
            </template>
          </el-table-column>
          <el-table-column label="已办结(件)">
            <el-table-column label="属实" width="60">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.allDoneReal, {
                      state: 9,
                      trueFlag: '1,2,3',
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.allDoneReal }}</span>
              </template>
            </el-table-column>
            <el-table-column label="不属实" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.allDoneFake, {
                      state: 9,
                      trueFlag: 4,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.allDoneFake }}</span>
              </template>
            </el-table-column>
            <el-table-column label="合计" width="80">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="
                    handleSee(scope.row.city, scope.row.allDone, {
                      state: 9,
                      followFlag: 0,
                    })
                  "
                >{{ scope.row.allDone }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="责令整改(家)" width="110px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="handleSee(scope.row)"
              >{{ scope.row.orderRectification }}</span>
            </template>
          </el-table-column>
          <el-table-column label="立案处罚(家)" width="110px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="handleSee(scope.row)"
              >{{ scope.row.initiatePunishment }}</span>
            </template>
          </el-table-column>
          <el-table-column label="罚款金额(家)" width="110px">
            <template v-slot="scope">
              <span class="accounChunk" @click="handleSee(scope.row)">{{ scope.row.fines }}</span>
            </template>
          </el-table-column>
          <el-table-column label="立案侦查(家)" width="110px">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="handleSee(scope.row)"
              >{{ scope.row.initiateInvestigation }}</span>
            </template>
          </el-table-column>
          <el-table-column label="拘留(人)">
            <el-table-column label="行政" width="90">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="handleSee(scope.row)"
                >{{ scope.row.administrativeDetention }}</span>
              </template>
            </el-table-column>
            <el-table-column label="刑事" width="90">
              <template v-slot="scope">
                <span
                  class="accounChunk"
                  @click="handleSee(scope.row)"
                >{{ scope.row.criminalDetention }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column label="约谈(家)" width="90">
            <template v-slot="scope">
              <span class="accounChunk" @click="handleSee(scope.row)">{{ scope.row.questioning }}</span>
            </template>
          </el-table-column>
          <el-table-column label="问责" width="90">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="
                  handleAccountabilitySee(
                    scope.row.city,
                    scope.row.accountability,
                    { type: 1 }
                  )
                "
              >{{ scope.row.accountability }}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div v-if="letterShow" style="margin-top: 30px">
        <el-table v-loading="letterLoading" :data="letterList" border :stripe="true">
          <el-table-column type="index" label="序号" width="50">
            <template scope="scope">
              <span>
                {{
                (letterQueryParams.pageNo - 1) * letterQueryParams.pageSize +
                scope.$index +
                1
                }}
              </span>
          ...
          <el-table-column label="举报主要内容" min-width="200px" :show-overflow-tooltip="true">
            <template v-slot="scope">
              <span
                class="accounChunk"
                @click="handleLetterSee(scope.row)"
              >{{ scope.row.tipOffContent }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="cityName" label="督察地市" width="120px"></el-table-column>
          <el-table-column prop="address" label="举报地址" width="160px" :show-overflow-tooltip="true"></el-table-column>
          <el-table-column
            prop="pollutionType"
            label="污染类型"
            width="120px"
          ...
          <el-table-column prop="toUnit" v-if="followFlagBoolean" label="责任单位" width="160px"></el-table-column>
          <el-table-column
            prop="transfetNumber"
            v-if="followFlagBoolean"
            label="转办编号"
            width="200px"
          ></el-table-column>
          <el-table-column prop="transferTime" v-if="followFlagBoolean" label="转办时间" width="160px"></el-table-column>
          <el-table-column
          ...
          <el-table-column
            ...
            width="120px"
          ></el-table-column>
          <el-table-column prop="handleTime" v-if="followFlagBoolean" label="办理时间" width="160px"></el-table-column>
          <el-table-column
            prop="finishFlagName"
            v-if="followFlagBoolean"
            label="办结状态"
        ...
        <pagination
          v-show="letterTotal > 0"
          :total="letterTotal"
          :pageNo.sync="letterQueryParams.pageNo"
          :limit.sync="letterQueryParams.pageSize"
          @pagination="getQueryListForhandle"
        />
      </div>
      <div v-if="accounShow" style="margin-top: 30px">
        <el-table :data="accounList" v-loading="accountLoading" style="width: 100%">
          <el-table-column type="index" label="序号" width="50">
            <template scope="scope">
              <span>{{ scope.$index + 1 }}</span>
            </template>
    ...
    >
        ...
        <div class="info-item">
          <div class="info-item-quare">
            ...
            <span class="info-item-content">
              ...
              manageInfo.pollutionType.split(",")[0]
              }}
            </span>
          </div>
        </div>
        <div class="info-item">
          <div class="info-item-quare">
            ...
            <span class="info-item-content">
              {{
              manageInfo.pollutionType &&
              manageInfo.pollutionType.split(",")[1]
              }}
            </span>
          </div>
        </div>
      ...
      <template v-if="repeatInfoBoolean">
        <div class="examineInfo">
          <h1 class="examineInfo-title">{{repetitionText}}</h1>
        </div>
        <div class="manage-info">
...
export default {
  ...
  created() {
    if (this.inspectInfo.suite) {
      ...
      this.handleTypeChange(superviseSubject.superviseType);
    }
    this.getList();
  },
  computed: {
  ...
  methods: {
    handleOverseersClick() {
      this.buttonShow = false;
      this.letterShow = false;
      this.accounShow = false;
    },
    ...
    async getList() {
      this.loading = true;
      const reponse = await anaInfoByTeam(this.queryParams);
      this.List = reponse.data;
      this.loading = false;
    },
    onQuery() {
      this.getList();
      this.handleOverseersClick();
    },
    onClear() {
    ...
    async handleSee(city, value, params) {
      if (value > 0) {
        this.buttonShow = true;
        this.letterShow = true;
        this.letterQueryParamsObj.city = city;
        this.letterQueryParamsObj.params = params;
        const { followFlag } = params;
        if (followFlag == 1) {
          this.followFlagBoolean = false;
        } else {
          this.followFlagBoolean = true;
        }
        this.getQueryListForhandle();
      }
    },
    async getQueryListForhandle() {
      this.letterLoading = true;
      const reponse = await queryListForhandle({
        city: this.letterQueryParamsObj.city,
        ...this.letterQueryParamsObj.params,
        ...this.letterQueryParams,
        teamId: this.queryParams.teamId,
        roundId: this.queryParams.roundId,
        batchId: this.queryParams.batchId
      });
      this.letterList = reponse.data.list;
      this.letterTotal = reponse.data.count;
      this.letterLoading = false;
    },
    async handleAccountabilitySee(city, value, params) {
      if (value > 0) {
        this.buttonShow = true;
        this.accounShow = true;
        this.letterQueryParamsObj.city = city;
        this.letterQueryParamsObj.params = params;
        this.getSourceList();
      }
    ...
    async getSourceList() {
      ...
      const reponse = await sourceList({
        city: this.letterQueryParamsObj.city,
        ...this.letterQueryParamsObj.params,
        ...this.queryLogParams,
        teamId: this.queryParams.teamId,
      ...
      this.accounTotal = reponse.data.count;
      this.accountLoading = false;
    },
    async handleLetterSee(data) {
      this.lookOpen = true;
    ...
    async getRepeatNumList(ids, repeatFlag) {
