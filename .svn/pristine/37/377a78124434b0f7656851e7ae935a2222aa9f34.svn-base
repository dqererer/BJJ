<template>
  <div>
    <div class="information-top" v-if="inspectBoolean">
      <p class="information-top-content">
        当前开展的督察为：<span class="turn-text">
          {{ inspectInfo.suite.superviseName }}<i>|</i></span
        >
        ...
        <span class="turn-text" v-if="areaName"><i>|</i>{{ areaName }}</span>
      </p>
    </div>
    <div class="information-top" v-else>
      <p class="information-top-content">
        当前开展的督察为：<span class="turn-text">
          {{ superviseItem.superviseName }}{{ superviseItem.roundName
          }}{{ superviseItem.batchName }}</span>
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (this.inspectInfo.groupName) {
      ...
      this.$emit("update:inspectName", this.inspectInfo.groupName[0].teamName);
      ...
      const superviseTemp = {
        superviseName: superviseSubject.superviseName,
        roundName: superviseSubject.roundName,
        batchName: superviseSubject.batchName
      };
      this.superviseItem = superviseTemp;
    }
  },
  computed: {
  ...
  methods: {
    handleNameChange(index) {
      ...
      this.$emit("update:inspectName", subject.teamName);
