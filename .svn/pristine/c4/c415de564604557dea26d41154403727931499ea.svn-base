    ...
    <section>
      ...
      <el-descriptions :column="2" border>
        <el-descriptions-item label="下沉组名称">{{
          tInspectProblem.xcTeamInfo && tInspectProblem.xcTeamInfo.name
        }}</el-descriptions-item>
        <el-descriptions-item label="下沉时间">
          {{
            tInspectProblem.xcTeamInfo &&
            tInspectProblem.xcTeamInfo.xcStartDate
          }}至{{
            tInspectProblem.xcTeamInfo && tInspectProblem.xcTeamInfo.xcEndDate
          }}
        </el-descriptions-item>
        <el-descriptions-item label="下沉旗县">
          {{
            tInspectProblem.xcTeamInfo && tInspectProblem.xcTeamInfo.areaName
          }}
        </el-descriptions-item>
      </el-descriptions>
    </section>
    <section>
      ...
      <el-descriptions :column="2" border>
        <el-descriptions-item label="问题名称">{{
          tInspectProblem.problem && tInspectProblem.problem.problemDescript
        }}</el-descriptions-item>
        <el-descriptions-item label="问题描述">{{
          tInspectProblem.problem && tInspectProblem.problem.problemContent
        }}</el-descriptions-item>
...
export default {
  ...
  methods: {
    ...
    getinfo: async function () {
      ...
      this.teamInfo = res.data.tInspectProblem.xcTeamInfo;
      this.dcInfo = JSON.stringify(res.data.tInspectProblem.xcTeamInfo);
      ...
      if (res.data.tInspectProblem.szList) {
        const clewArry = [];
        const mainList = res.data.tInspectProblem.szList;
        for (const item in mainList) {
          const subject = mainList[item];
          clewArry.push({
            clueName: subject.title,
            clueId: subject.id,
          });
        }
        this.szList = clewArry;
      }
