      ...
      <el-table v-loading="loading" :data="List" border :stripe="true">
        ...
        <el-table-column prop="fileReport" label="报告预览" width="160px"
          ><template v-slot="scope">
            <span
              class="accounChunk"
              @click="preview(scope.row.fileReport)"
...
import { getdownLoadZip, getSingleInfo, filePreview } from "@/utils/styem";
...
export default {
  data() {
    var validOption = (rule, value, callback) => {
      const { levelType, inspectType, roundId, batchId } = this.dialogParams;
      if (levelType && inspectType && roundId && batchId) {
        callback();
        return;
      } else {
  ...
  methods: {
    ...
    handleItem: function (val) {
      this.$set(this.dialogParams, "inspectType", val[1]);
      this.$set(this.dialogParams, "roundId", val[2]);
      this.$set(this.dialogParams, "batchId", val[3]);
      let parmas = {
        levelType: this.dialogParams.levelType,
        inspectType: val[1],
        roundId: val[2],
        batchId: val[3],
      };
      getAllChangeProgramList(parmas).then((res) => {
        this.superviseNameArry2 = res.data;
        this.$set(this.dialogParams, "planId", undefined);
      });
    },
    getNodes(val) {
      if (val.length == 1) {
        getSpectTypeList({ parentId: val[0] }).then((res) => {
          this.superviseLevelArry2.forEach((item) => {
            if (item.id == val[0]) {
              this.$set(this.dialogParams, "levelType", item.value);
              item.children =
                res.data &&
                res.data.map((i) => {
                  return {
    ...
    async handleEdit(info) {
      ...
      getSpectTypeList({ parentId: 0 }).then((res) => {
        ...
        this.setSuperviseLevelArry2();
        let parmas = {
          levelType: info.value,
          inspectType: info.inspectType,
    ...
    setSuperviseLevelArry2: function () {
      getSpectTypeList({ parentId: this.selectedOptions[0] }).then((res) => {
        this.superviseLevelArry2.forEach((item) => {
          if (item.id == this.selectedOptions[0]) {
            item.children =
              res.data &&
              res.data.map((i) => {
                return {
        ...
        getRoundOrBatchList({ superviseType: this.selectedOptions[1] }).then(
          (res) => {
            this.superviseLevelArry2.forEach((item) => {
              if (item.id == this.selectedOptions[0]) {
                item.children.forEach((element) => {
                  if (element.id == this.selectedOptions[1]) {
                    element.children =
                      res.data &&
                      res.data.map((i) => {
                        return {
            ...
            getRoundOrBatchList({ id: this.selectedOptions[2] }).then((res) => {
              this.superviseLevelArry2.forEach((item) => {
                if (item.id == this.selectedOptions[0]) {
                  item.children.forEach((element) => {
                    if (element.id == this.selectedOptions[1]) {
                      element.children.forEach((o) => {
                        if (o.id == this.selectedOptions[2]) {
                          o.children =
                            res.data &&
                            res.data.map((i) => {
                              return {
