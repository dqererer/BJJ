    ...
    <div class="tab-list-content">
      <div class="box-body" v-show="showSearch">
        <el-form :model="queryParams" label-width="120px">
            ...
            <el-col :span="8">
              <div class="line3">
                <el-form-item label="整改目标：">
                  <el-input
                    placeholder="请输入整改目标"
                    v-model="queryParams.rectifyTarget"
                  >
                  </el-input>
                </el-form-item>
              </div>
            ...
            <el-col :span="8">
              <div class="line3">
                <el-form-item label="完成情况：">
                  <el-select
                    v-model="queryParams.finishSituation"
                    placeholder="请选择完成情况"
                  >
                    <el-option
          ...
          <el-row>
            <el-col :span="8" v-if="userInfo.userType == '1'">
              <div class="line3">
                <el-form-item label="地区搜索：">
                  <el-select
                    v-model="queryParams.writeCityCode"
                    placeholder="请选择地区搜索"
                  >
                    <el-option
      ...
      <el-table v-loading="loading" :data="List" border :stripe="true">
        ...
        >
        <el-table-column prop="title" label="督察信息" min-width="300px">
          <template v-slot="scope">
            <span
              >{{ scope.row.levelTypeName }}-{{ scope.row.inspectTypeName }}-{{
                scope.row.roundName
        ...
        <el-table-column prop="reportDate" label="完成时限" width="160px"
          ><template v-slot="scope">
            <span v-if="scope.row.deadline == 'now'">立即立改</span>
            <span v-else-if="scope.row.deadline == 'longtime'">长期坚持</span>
        ...
        <div class="wrap">
          ...
          <div class="right">
            ...
            <div class="card middle">
              <div class="title">
                整改措施（共{{ dialogParams.measureDTOList.length }}条）
              </div>
              <section style="padding-top: 10px">
                <el-tabs
        ...
        >
          ...
          <el-row :gutter="24">
            <el-col :span="24">
              <el-form-item
                label="核实内容："
                prop="rectifyVerify.verfityContent"
              >
                <el-input
                  class="inputWidth"
                  type="textarea"
          ...
          <el-row :gutter="24">
            <el-col :span="24">
              <el-form-item
                label="核实意见："
                prop="rectifyVerify.verfitySuggestion"
              >
                <el-input
                  class="inputWidth"
                  type="textarea"
          ...
          <el-form-item
            label="核实情况："
            prop="rectifyVerify.verfitySituation"
          >
            <el-radio-group
  ...
  methods: {
    ...
    toggleStep: function (idx) {
      ...
      this.$nextTick(() => {
        ...
        let str =
          this.dialogParams.measureDTOList[this.proIndex].progressDTOList[
            this.proIndex
          ].file;
        let arr = str ? str.split(",") : [];
        this.curFileNum = arr.length;
      });
    },
    ...
    handleFinishStauts: function (v) {
      if (v == "1") {
        this.$confirm(
          "本措施完成后，对应措施也将变更为已完成，次月将不能再进行任务填报，是否确认修改",
          "提示",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
        ...
        if (
          ...
          if (
            this.dialogParams.measureDTOList[this.tabIndex].finishState == "1"
          ) {
            this.$confirm(
              "本措施未完成后，对应措施也将修改为未完成, 是否确定未完成?",
              "提示",
              {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
    ...
    onClear() {
      ...
      this.queryParams.writeCityCode = undefined;
      this.queryParams.finishSituation = undefined;
      this.queryParams.rectifyContent = undefined;
      this.queryParams.rectifyTarget = undefined;
    ...
    async dailogSubmit() {
      ...
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          this.dialogParams.meetingMinutesFileIds = saveChangeTask(
            this.dialogParams
          ).then((res) => {
            if (res.code == "200") {
              this.$message({
    ...
    setUrlPath(file) {
      this.handleContentChange();
      let arr = this.dialogParams["measureDTOList"][this.tabIndex]
        .progressDTOList[this.proIndex][file.fromKey]
        ? this.dialogParams["measureDTOList"][this.tabIndex].progressDTOList[
            this.proIndex
          ][file.fromKey].split(",")
        : [];
      arr.push(file.fileId);
      let str = arr.join(",");
      this.$set(
        this.dialogParams["measureDTOList"][this.tabIndex].progressDTOList[
          this.proIndex
    ...
    handleTabsEdit(targetName, action) {
      if (action === "add") {
        let len = this.dialogParams.measureDTOList.length;
        let newTabName = "措施" + (len + 1);
        this.dialogParams.measureDTOList.push({
          measure: "",
          finishState: "",
      ...
      getProgressList({
        ...
        if (res.code == "200") {
          ...
          this.$nextTick(() => {
            this.$refs.uploadFile && this.$refs.uploadFile.getFileMessageInfo();
            let str =
              this.dialogParams.measureDTOList[index].progressDTOList[
                this.proIndex
              ].file;
            let arr = str ? str.split(",") : [];
            this.curFileNum = arr.length;
