...
export default {
  ...
  methods: {
    ...
    toggleType: function (newV) {
      console.log(this.dczInfo);
      switch (newV) {
        case "督察人员":
          this.$router.push({
            path: "/renyuan",
            query: { id: this.id, superviseOne: this.dczInfo.superviseOne },
          });
          break;
        case "相关线索":
        ...
        case "进驻前资料":
          this.$router.push({
            path: "/jzqziliao",
            query: { batchId: this.dczInfo.batchId },
          });
          break;
        case "进驻会议":
