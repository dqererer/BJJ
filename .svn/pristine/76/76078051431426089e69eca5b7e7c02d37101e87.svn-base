  ...
  <div>
    ...
    >
      <el-button
        ...
        >预览</el-button
      >
      <examineInfo2 v-if="isSiglePage" :dcInfo="dcInfo" />
      <examineInfo v-else />
...
import examineInfo2 from "@/components/examineInfo2";
import examineInfo from "@/components/examineInfo2";
...
import inspectPreview from "../../../burg/garrison/sink/sinkIssue/inspectPreview";
...
export default {
  ...
  data() {
    return {
      ...
      dialogRecordRules: {
        theme: [{ required: true, message: "请输入问询主题", trigger: "blur" }],
        startTime: [
          { required: true, message: "请选择谈话时间", trigger: "change" },
        ],
        actualSpeakerIds: [
          { required: true, message: "请选择谈话人员", trigger: "change" },
        ],
        actualRecordId: [
          { required: true, message: "请选择记录人员务", trigger: "change" },
        ],
        actualInterviewee: [
          { required: true, message: "请输入被谈话人", trigger: "blur" },
        ],
        ...
        audioId: [
          { required: true, message: "请上传录音文件", trigger: "change" },
        ],
  ...
  methods: {
    handleInspectArry() {
      ...
      this.dialogTitle = "新增询问笔录";
      this.handleformClear("dialogRecordForm");
      this.inspectArryOther = this.list[this.tabIndex].inspectArry;
    ...
    dailogRecordSubmit() {
      this.$refs.dialogRecordForm.validate(async (valid) => {
        if (valid) {
          ...
          this.dialogRecordParams.actualSpeakerIds =
            this.dialogRecordParams.actualSpeakerIds.join(",");
          ...
          this.$emit("handleInspectClose", {
            inspectArryOther: this.inspectArryOther,
      ...
      this.$refs.dialogClewForm.validate(async (valid) => {
        if (valid) {
          ...
          const clewTemp = {
            ...
            indentId: this.dialogRecordParams.id,
          };
    ...
    async handleEdit() {
      ...
      this.dialogTitle = "修改询问笔录";
      this.stateText = "修改";
      if (this.curStatus == "view") {
        this.dialogTitle = "查看询问笔录";
      }
      // this.inspectArryOther = this.list[this.tabIndex].inspectArry;
      // this.otherClewList = this.list[this.tabIndex].clewList;
